...
The PTR-94 design, while thermodynamically constrained, opens a number of testable research questions across multiple disciplines.

\textbf{Stoichiometry ceiling.} What is the absolute upper bound on H$^+$ pumped per NADH, given the full free energy landscape of electron transfer and the need to maintain a membrane potential sufficient for ATP synthesis? Extending the naturally observed ~10~H$^+$/NADH to the ~30~H$^+$/NADH required by the PCM demands an expanded membrane domain with 8--10 independent proton channels~\cite{sazanov2015complex}.

\textbf{Kinetic constraints.} How do finite reaction rates and activation barriers reduce the theoretically achievable yield? At turnover rates exceeding $10^3$~ATP/s per complex, the free energy dissipated to overcome kinetic barriers may reduce net ATP yield by 15--25\% relative to the thermodynamic limit.

\textbf{Membrane integrity.} Can synthetic membranes — archaeal tetraether lipids, polymer-stabilized bilayers, or hybrid systems — sustain a proton motive force of 250--300~mV indefinitely without dielectric breakdown, excessive leak, or structural failure?

\textbf{ROS elimination.} Is it physically possible to design an electron transfer chain that produces zero reactive oxygen species, or does quantum tunneling of O$_2$ into active sites guarantee an irreducible leak rate of $\sim10^{-6}$ per electron transfer?

\textbf{Evolutionary accessibility.} Does the fitness landscape separating natural ATP yields (30--38~ATP/glucose) from the PTR-94 target contain viable intermediate states, or does the transition require crossing a fitness valley that necessitates de novo design rather than directed evolution?

\textbf{Direct phosphorylation.} Can redox-driven conformational changes be harnessed for ATP synthesis without an intervening delocalized proton gradient, and if so, does this bypass the membrane integrity constraints of chemiosmotic coupling?

\textbf{Practical cellular limit.} Integrating all thermodynamic, kinetic, structural, and regulatory constraints, what is the maximum ATP yield achievable in a genetically encodable, self-replicating system? The practical limit may be 55--65~ATP/glucose — approximately 2$\times$ natural respiration but still well below the 94~ATP theoretical ceiling.

These questions, and a detailed framework for addressing them experimentally and computationally, are explored in the companion \textit{Research Questions} document in the repository.
...
\bibitem{boyer1997atp}
P.~D.~Boyer, ``The ATP synthase -- a splendid molecular machine,'' \textit{Annu. Rev. Biochem.}, vol.~66, pp.~717-749, 1997.

\bibitem{abrahams1994structure}
J.~P.~Abrahams, A.~G.~W.~Leslie, R.~Lutter, and J.~E.~Walker, ``Structure at 2.8~\AA{} resolution of F$_1$-ATPase from bovine heart mitochondria,'' \textit{Nature}, vol.~370, pp.~621-628, 1994.

\bibitem{noji1997rotation}
H.~Noji, R.~Yasuda, M.~Yoshida, and K.~Kinosita~Jr., ``Direct observation of the rotation of F$_1$-ATPase,'' \textit{Nature}, vol.~386, pp.~299-302, 1997.

\bibitem{sazanov2015complex}
L.~A.~Sazanov, ``A giant molecular proton pump: structure and mechanism of respiratory complex I,'' \textit{Nat. Rev. Mol. Cell Biol.}, vol.~16, pp.~375-388, 2015.

\bibitem{brand2000uncoupling}
M.~D.~Brand, ``Uncoupling to survive? The role of mitochondrial inefficiency in ageing,'' \textit{Exp. Gerontol.}, vol.~35, pp.~157-164, 2000.
